...
export default function Navbar() {
  ...
  if (isLoginPage) {
    return (
      <nav className="bg-white border-b border-stone-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 md:h-16 flex items-center">
          <div className="flex items-center gap-2">
            <HouseLogo />
            <span className="text-lg md:text-xl font-semibold text-stone-800" style={{ fontFamily: 'Playfair Display, serif' }}>
              Home Wellness
            </span>
          </div>
        </div>
  ...
  return (
    <nav className="bg-white border-b border-stone-200 sticky top-0 z-50" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
            ...
            {user && (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 px-2 md:px-3 py-1.5 rounded-xl border border-stone-200 hover:bg-stone-50 transition-colors active:bg-stone-100"
                >
                  {user.avatar ? (
                  ...
                  <span className="hidden md:block text-sm font-medium text-stone-700">{user.name.split(' ')[0]}</span>
                  <svg className={`hidden md:block w-4 h-4 text-stone-400 transition-transform ${profileOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
